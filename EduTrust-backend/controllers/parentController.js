    ...
    try {
        ...
        if (
            ...
            return res.status(400).json({
                success: false,
                message: "First name, last name, relationship, phone, school, and password are required."
            });
        }
        ...
        const cleanOtherName = other_name?.trim() || "";
        ...
        if (cleanEmail) {
            duplicateParentConditions.push({ school_id, email: cleanEmail });
        }

        const existingParent = await Parent.findOne({
            $or: duplicateParentConditions
        });
        ...
        try {
            ...
            const fullName = [cleanFirstName, cleanOtherName, cleanLastName]
                .filter(Boolean)
                .join(" ");
            ...
            // Roll back parent profile creation on user model failure
...
exports.loginParent = async (req, res) => {
    try {
        ...
        const parent = await Parent.findOne({
            $or: [{ email: normalizedInput }, { phone: targetIdentity }],
            status: "Active"
        }).lean();
        ...
        if (!parent) {
            return res.status(404).json({
                success: false,
                message: "No active parent account found with these credentials."
            });
        }
        ...
        if (!user) {
            return res.status(401).json({
                success: false,
                message: "Parent login credentials not found."
            });
        }

        // 3. Verify Role & Active Status
        ...
        // 5. Verify Active School Status
        const school = await School.findById(parent.school_id).lean();
        if (!school || school.status !== "Active") {
            return res.status(403).json({
                success: false,
                message: "Associated school account is inactive or missing."
            });
        }
...
exports.updateParentProfile = async (req, res) => {
    try {
        ...
        if (relationship !== undefined) updateFields.relationship = relationship.trim();
...
exports.getParents = async (req, res) => {
    try {
        ...
        const formattedParents = parents.map(parent => ({
            ...parent,
            student_count: countMap.get(String(parent._id)) || 0
        }));
...
exports.updateParentById = async (req, res) => {
    try {
        ...
        const updatedParent = await Parent.findOneAndUpdate(
            { _id: req.params.id, school_id: schoolId },
            { $set: updateFields },
            { new: true, runValidators: true }
        ).lean();
...
exports.getParentDashboard = async (req, res) => {
    try {
        ...
        const suspendedChildren = children.filter(c => c.status === "Suspended").length;
